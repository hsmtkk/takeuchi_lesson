...
def init_messages():
    ...
    if clear_button or "messages" not in st.session_state:
        welcome_message = (
            "ベアーモバイル カスタマーサポートへようこそ。ご質問をどうぞ🐻"
        )
...
def select_model():
    models = ("Gemini 2.5 Flash Lite", "OpenRouter")
    model = st.sidebar.radio("Choose a model:", models)
    if model == "Gemini 2.5 Flash Lite":
        return ChatGoogleGenerativeAI(temperature=0, model="gemini-2.5-flash-lite")
    elif model == "OpenRouter":
        return ChatOpenAI(
            model="google/gemma-3-27b-it:free", base_url="https://openrouter.ai/api/v1"
        )
